...
public class ActTools {
    ...
    public static RequestInter get(FragmentActivity activity) {
        ...
        if (fragment == null) {
            RequestFragmentSupport requestFragmentSupport = RequestFragmentSupport.newInstance();
            fm.beginTransaction().add(requestFragmentSupport, TAGCompat).commitAllowingStateLoss();
            fm.executePendingTransactions();
            return requestFragmentSupport;
        } else {
            return ((RequestFragmentSupport) fragment);
        }
    }
